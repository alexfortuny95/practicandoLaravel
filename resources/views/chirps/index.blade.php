                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('chirps.store') }}">
                        ...
                        <textarea
                            ...
                            name="message" id="" placeholder="{{ __('What\'s on your mind?') }}">{{ old('message') }}</textarea>
                            ...
                            <x-dropdown>
                                ...
                                <x-slot name="content">
                                    ...
                                    <form method="POST" action="{{ route('chirps.destroy', $chirp) }}">
                                        @csrf @method('DELETE')
                                        <x-dropdown-link onclick="event.preventDefault(); this.closest('form').submit();" :href="route('chirps.destroy', $chirp)">{{ __('Delete Chirp') }} </x-dropdown-link>
                                    </form>
                                </x-slot>
                            </x-dropdown>
                        @endcan
